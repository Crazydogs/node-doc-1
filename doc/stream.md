...
Transform 流是输出以某种方式依赖于输入的[双工流](#streamduplex-类)。
作为[双工流](#streamduplex-类)，他也实现了[可读](#streamreadable-类)
...
| 读写                |[Duplex](#streamduplex-类)    |[_read](#readable_readsize), [_write](#writable_writechunk-encoding-callback), [_writev](#writable_writevchunks-callback) |
| 写数据读结果        |[Transform](#streamtransform-类)         |[_transform](#transform_transformchunk-encoding-callback), [_flush](#transform_flushcallback)     |

注意：实现中请不要调用流模块的"公用"方法(在 [使用流涉及的 API](#使用流涉及的-api) 部分中讲到的)。
...
- 参数 `options` \<Object\>
    - `highWaterMark` \<Number\> 指定缓存数量达到什么水平的时候 [stream.write()](#writablewritechunk-encoding-callback)
    开始返回 `false`。默认是 `16384`(16kb) 或对象模式的流是 `16`(个对象)
    - `decodeStrings` \<Boolean\> 指定是否在将字符串数据传给 [_write](#writable_writechunk-encoding-callback) 方法前解码，
    默认为 `true`
    - `objectMode` \<Boolean\> 决定调用 [stream.write(anyObj)](#writablewritechunk-encoding-callback) 是否合法。如果设置了此属性，
    则可以向流中写入除了字符串和 `Buffer` 对象之外的其他 JavaScript 值。默认为
    `false`
    - `write`\ <Function\> 对 [stream._write()](#writable_writechunk-encoding-callback) 的实现
    - `writev` \<Function\> 对 [stream._writev()](#writable_writevchunks-callback) 的实现
...
所有的可写流都必须提供 [stream._write()](#writable_writechunk-encoding-callback) 的实现，用于将数据写入底层。

注意：[Transform](#streamtransform-类) 流会提供自己的 [stream._write()](#writable_writechunk-encoding-callback) 函数实现。
...
- 参数 `options` \<Object\>
    ...
    默认值为 `null`
    - `objectMode` \<Boolean\> 流是否工作在流模式下。即调用 [stream.read(n)](#readablereadsize) 返回一个值，
    而不是返回指定长度的字符串或 `Buffer` 对象。
    - `read` \<Function\> 对 [stream._read()](#readable_readsize) 的实现
...
当 `readable._read()` 被调用时，如果底层数据可用，则应该通过 [this.push(dataChunk)](#readablepushchunk-encoding)
方法将数据推入可读流的缓冲队列中。`_read()` 将持续的将数据汲取到缓冲队列中，
直到 `readable.push()` 返回 `false`。再次调用 `_read()` 将使其重新开始重复以上过程。

注意：`readable._read()` 被调用后，直到调用 [readable.push](#readablepushchunk-encoding) 之前，都不会再次被调用。

`size` 参数并不是强制性的。对于读取操作是简单地返回数据的情况，可以用 `size`
参数来决定要提取多少数据。其他情况下可以直接忽略此参数，当有数据可用就直接返回。
没有必要等到可用数据达到 `size` 指定大小再去调用 [stream.push(chunk)](#readablepushchunk-encoding)
...
如果流处在暂停模式下，那么使用 `readable.push()` 添加的数据在 [readable](#readable-事件) 事件触发后，
可以通过调用 [readable.read()](#readablereadsize) 方法获取。
...
[双工流](#streamduplex-类)是同时可读写的流，如 TCP socket 连接。
...
Transform 流是一种[双工流](#streamduplex-类)，其输出是根据输入计算得到的。比如用于压缩数据的 zlib
流，加密解密数据的 crypto 流等。
...
方法和 `readable._read()` 方法。自定义的 Transform 流实例必须实现 [transform._transform()](#transform_transformchunk-encoding-callback)
方法，可以实现可选的 [transform.flush()](#transform_flushcallback) 方法。
...
[finish](#finish-事件) 事件和 [end](#end-事件) 事件分别来自 [stream.Writable](#streamwritable-类)
和 [stream.Readable](#streamreadable-类) 类。`fiinsh` 事件会在调用 [stream.end()](#stream_writable_end_chunk_encoding_callback)
之后触发。`end` 事件会在所有数据输出，并在 [transform._flush()](#stream_transform_flush_callback) 调用之后触发。
...
自定义 [Transform](#streamtransform-类) 流实现可以实现 `transform._flush()` 方法。 当没有更多写入数据以供消耗时，
在发射 [end](#end-事件) 事件通知可读流结束之前，将调用该方法。
...
- 之前的可读流不会等待调用 [stream.read()](#readablereadsize) 方法，[data](#data-事件)
事件会立即开始发射。须要做一些额外的工作，接收数据并保存到缓冲区，防止数据流失。
- [stream.pause()](#readablepause) 方法并不能保证使流暂停。这意味着即使是暂停的流，也有必要做好接收数据的准备。

在 Node.js v0.10 中添加了 [Readable](#streamreadable-类) 类。为了和旧的 Node.js 程序兼容，
可读流在添加 [data](#data-事件) 事件监听器或者调用 [stream.resume()](#readableresume)
方法时会切换到流模式。现在即使不添加额外的 [readable](#readable-事件) 事件监听或者调用
[stream.read](#readablereadsize) 方法，也不用担心数据丢失了。

虽然大部分的应用可以正常工作，但是在同时满足这些条件时会引入边界情况：

- 没有添加任何 [data](#data-事件) 事件的监听。
- [stream.resume()](#readableresume) 方法没有被调用。
- 流没有被 pipe 到任何可写流。
...
将零字节字符串或缓冲区推送到非对象模式的流有一个有趣的副作用。因为它是对
[readable.push()](#readablepushchunk-encoding) 的调用，调用将结束读取过程。
但是，因为参数是一个空字符串，所以并没有数据被添加到可读缓冲区，所以没有什么数据可供消费。
